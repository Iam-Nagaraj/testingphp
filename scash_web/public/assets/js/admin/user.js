$(document).ready(async function () {
	...
	let columns = [
		...
		},
		{
			...
			orderable: false,
			render: (data, type, row, meta) => {
				return  data.country_code + '&nbsp;' + data.phone_number;
			}
		},
		{ title: 'Email', data: 'email' },
		{
			...
			orderable: false,
			render: (data, type, row, meta) => {
				return data.address?data.address.city:'N/A';
			}
		},
		{
	...
	await InitDataTable('users-table', user_datatable_url, columns);
	...
	const userStatus = async (id, status) => {
		if (confirm('Are You sure?')) {
			...
			let url = user_status_change_url;
			const options = {
				type: "POST",
				url: url,
				data: fromData,
	...
	$(document).on('click', '.user-list-tab', async function () {
		let $this = $(this);
		let status = $this.attr('data-type');

		await InitDataTable('users-table', user_datatable_url, columns, status);
	})
