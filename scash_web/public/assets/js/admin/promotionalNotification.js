$(document).ready(async function () {
	...
	await InitDataTable('promotionalNotification-table', promotionalNotification_datatable_url, columns);

	const statusButton = document.querySelector('.status-action');
	if (statusButton) {
		statusButton.addEventListener('change', (element) => {
			promotionalNotificationStatus($(element.target).attr('data-id'), $(element.target).attr('data-status'));
		});
	}

	const promotionalNotificationStatus = async (id, status) => {
		if (confirm('Are You sure?')) {
			const fromData = { id: id, status: (status == active_status ? inactive_status : active_status) };
			let url = promotionalNotification_status_change_url;
			const options = {
				type: "POST",
				url: url,
				data: fromData,
	...
	$(document).on('click', '.promotionalNotification-list-tab', async function () {
		let $this = $(this);
		let status = $this.attr('data-type');

		await InitDataTable('promotionalNotification-table', promotionalNotification_datatable_url, columns, status);
	})

})

$(document).on('submit', '#promotionalNotification-form', function (e) {
	e.preventDefault();
	let myForm = document.getElementById('promotionalNotification-form');
	let target = "promotionalNotification-body";
...
async function AjaxFromLoginSubmit(formData, url, method, target) {
	...
	$.ajax({
		...
		success: async function (response) {
			...
			await new Promise(resolve => setTimeout(resolve, 2500));
			navigateTo('/promotional-notification');
