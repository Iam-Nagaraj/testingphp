$(document).ready(async function () {
	...
	let columns = [
		...
		{ title: 'Dwolla Key', data: 'dwolla_key' },
		{
			title: 'Action',
			data: 'action',
			orderable: false,
	...
	await InitDataTable('businessType-table', businessType_datatable_url, columns);

	const statusButton = document.querySelector('.status-action');
	if (statusButton) {
		statusButton.addEventListener('change', (element) => {
			businessTypeStatus($(element.target).attr('data-id'), $(element.target).attr('data-status'));
		});
	}

	const businessTypeStatus = async (id, status) => {
		if (confirm('Are You sure?')) {
			const fromData = { id: id, status: (status == active_status ? inactive_status : active_status) };
			let url = businessType_status_change_url;
			const options = {
				type: "POST",
				url: url,
				data: fromData,
	...
	$(document).on('click', '.businessType-list-tab', async function () {
		let $this = $(this);
		let status = $this.attr('data-type');

		await InitDataTable('businessType-table', businessType_datatable_url, columns, status);
	})

});

$(document).on('submit', '#businessType-form', function (e) {
	e.preventDefault();
	let myForm = document.getElementById('businessType-form');
	let target = "businessType-body";
