$(document).ready(async function () {
	...
	await InitDataTable('businessCategory-table', businessCategory_datatable_url, columns);

	const statusButton = document.querySelector('.status-action');
	if (statusButton) {
		statusButton.addEventListener('change', (element) => {
			businessCategoryStatus($(element.target).attr('data-id'), $(element.target).attr('data-status'));
		});
	}

	const businessCategoryStatus = async (id, status) => {
		if (confirm('Are You sure?')) {
			const fromData = { id: id, status: (status == active_status ? inactive_status : active_status) };
			let url = businessCategory_status_change_url;
			const options = {
				type: "POST",
				url: url,
				data: fromData,
	...
	$(document).on('click', '.businessCategory-list-tab', async function () {
		let $this = $(this);
		let status = $this.attr('data-type');

		await InitDataTable('businessCategory-table', businessCategory_datatable_url, columns, status);
	})

});

$(document).on('submit', '#businessCategory-form', function (e) {
	e.preventDefault();
	let myForm = document.getElementById('businessCategory-form');
	let target = "businessCategory-body";
